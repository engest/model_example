...
model_example::model_example(sc_module_name name) : sc_module(name)
                                          , tsocket("tsocket")
                                          , isocket("tsocket") {
    tsocket.register_b_transport(this, &model_example::b_transport);
}

void model_example::b_transport(tlm_generic_payload & trans, sc_time & delay) {
    while (true) {
        wait(start);
